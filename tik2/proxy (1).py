...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # Prepare the proxy socket. We will bind to socket, and wait and listen for incoming information/packets
        # * Fill in start (1)
        proxy_socket.bind(("0.0.0.0", proxy_address))
        ...
        while True:
            try:
                ...
                # TCP connection, hence we need to connect to socket
                # * Fill in start (2)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # Receive data from the client using the standard library method recv
            # * Fill in start (3)
