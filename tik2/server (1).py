...
DATA_MAX_SIZE = 8192 # as written in pdf
...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # # Prepare the server socket. We will bind to socket, and wait and listen for incoming information/packets
        # * Fill in start (1)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            # recieve information using the recv function
            # * Fill in start (3)
            data = client_socket.recv(DATA_MAX_SIZE)
            # * Fill in end (3)
            if not data:
                break
            try:
                ...
                # * Fill in start (4)
                # sending information using the sendall function (sends untill nothin left to send)
